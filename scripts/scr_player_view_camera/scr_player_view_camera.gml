function scr_player_view_camera(){			
	...
	var view_count = instance_place_list(x,y, objView, list_view, true);	
	scr_view_camera_variables();
		
	if(view_count == 1 and camera_lock){		
		var current_view = instance_place(x, y, objView);	
		view_x = x - camera_w/2;
		view_y = y - camera_h/2;
		camera_lock = true;
